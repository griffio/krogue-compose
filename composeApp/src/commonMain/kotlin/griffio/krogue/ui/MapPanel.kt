...
/** Shared style for the map grid; measured once to size the effects overlay. */
private val GridTextStyle = TextStyle(
 ...
 * recompose. The camera centres on the hero and clamps to the map edges. A
 * transparent [EffectsOverlay] is laid over the same grid for floating combat
    ...
    TerminalPanel(title = "DUNGEON · depth ${game.depth}", modifier = modifier) {
        ...
        Box(Modifier.padding(2.dp)) {
            ...
            EffectsOverlay(
                effects = effects,
                metrics = GridMetrics(
                    cellWidth = cell.size.width.toFloat(),
                    cellHeight = cell.size.height.toFloat(),
                    originX = originX,
                    originY = originY,
                ),
...
private fun buildMap(
    ...
    for (sy in 0 until rows) {
        ...
        for (sx in 0 until cols) {
            val x = originX + sx
            val monster = game.monsterAt(x, y)
            when {
